...
#define _500MS_TIMER	10
#define _4S_TIMER		80
#define _5S_TIMER		100
...
volatile int16_t ret = 0;
uint16_t data_ready;
uint8_t env_count = 0;
uint8_t aq_count = 0;
volatile int error = 0;
struct sps30_measurement meas;
struct gas_measurement gas_meas;
bool sps_available, aq_flag;
...
int main(void)
{
  ...
  SystemClock_Config();
  ALLMEMS1_PRINTF("Clocks configured\r\n");
  // Initialising I2C3
//   MX_I2C3_Init();
...
//  Set2GAccelerometerFullScale();
  ...
  // Setup SPS30, must be done first to initiate the I2C driver
  sps_init();

  // Defunct function
//  begin_mics(hi2c3);

  // Setup of MiCS-4514 sensor, checking state of sensor
  if( getPowerState(hi2c3) == SLEEP_MODE ){
	  wakeUpMode(hi2c3);
  }
  // warmUpTime(hi2c3, 1);

  int range = RANGE_TIME_WITHOUT_CONNECTED;
  ...
  while (1)
  {
...
#endif /* ALLMEMS1_ENABLE_SD_CARD_LOGGING */
    {
      ...
      // 2000s timeout
      if( ActivityTimeout_CurrTime - ActivityTimeout_StartTime > range )
      {
        MCU_PowerSave();
      }
    }
    ...
    if(SendAccGyroMag) {
      ...
      // Every 500ms enter loop
      if( env_count%_500MS_TIMER == 0 ){
    	  if( !aq_flag ){
      ...
      if( env_count%_4S_TIMER == 0 && env_count != 0 ){
    	  if ( !aq_flag ){
			  ret = sps30_start_measurement();
			  aq_flag = true;
		  }
      }
      if( env_count%_5S_TIMER == 0 && env_count != 0 ){
    	SendEnvironmentalData();
    	readMOXData();
...
void sps_init(){
	...
	// Setup sps30, 15 attempts
	if( !probing_sps30(15) ){
		ALLMEMS1_PRINTF("Probing SPS failed\r\n");
    	HAL_GPIO_WritePin(GPIOG, GPIO_PIN_9|GPIO_PIN_10, GPIO_PIN_SET);
	}
	else{
		ALLMEMS1_PRINTF("Probing SPS30 Successful\r\n");
		sps_available = true;
	}

	// Delay and setup auto cleaning interval
	HAL_Delay(500);
	...
	if( ret ){
	 error = 1;
	 ALLMEMS1_PRINTF("Error in cleaning interval setup: %d\r\n", ret);
	}
	HAL_Delay(20);

	sps30_read_measurement(&meas);
	HAL_Delay(20);
...
bool probing_sps30(uint8_t max_val){
	HAL_Delay(2000);
	uint8_t count = 0;

	while( sps30_probe() ){
		// After max_val attempts of probing sensor break from loop, declare sensor data not available
		count++;
		if( count > max_val ){
			sps_available = false;
			return false;
...
static void SendMotionData(void)
{
  ...
  // Logging the data being transmitted through USB Debugging port
#ifdef ALLMEMS1_DEBUG_NOTIFY_TRANSMISSION
  ...
  acc_x_to_send = ACC_Value.x;
  acc_y_to_send = ACC_Value.y;
  acc_z_to_send = ACC_Value.z;

  ALLMEMS1_PRINTF("ACC_X=%d ", acc_x_to_send);
  ALLMEMS1_PRINTF("ACC_Y=%d ", acc_y_to_send);
  ALLMEMS1_PRINTF("ACC_Z=%d \r\n", acc_z_to_send);
  ...
  *Temp2ToSend=0,*Temp1ToSend=0;
  ...
  if(TargetBoardFeatures.InitHumiditySensor)
  {
    ...
    *Temp1ToSend = intPart*10+decPart;
	  ...
	  // Read values from off-board sensor to gather humidity data
  }
...
/*
 *	Reading the PM Sensor data if currently available
 */
void readAQData(void)
{

	 // Check if SPS is declared as available
	 if( sps_available ){
		...
		if( ret < 0)
		{
			ALLMEMS1_PRINTF("Error reading data-ready flag, %d\r\n", ret);
			HAL_GPIO_WritePin(GPIOG, GPIO_PIN_9, GPIO_PIN_SET);
		}
		else if( ret ){
			ALLMEMS1_PRINTF("Data not ready, no new measurements available, %d\r\n", data_ready);
			HAL_GPIO_WritePin(GPIOG, GPIO_PIN_9, GPIO_PIN_SET);
			// ret = sps30_read_measurement(meas);
		}
		else{
		  // If the data is ready update the measurement struct and transmit via bluetooth
		  ret = sps30_read_measurement(&meas);
		  if( ret ){
			  HAL_GPIO_WritePin(GPIOG, GPIO_PIN_9, GPIO_PIN_SET);
		  }
...
void readMOXData(){
	getAllSensorData(hi2c3, &gas_meas);
//	ALLMEMS1_PRINTF("CO:\t%d\r\nCH4:\t%d\r\nH2:\t%d\r\nNH3:\t%d\r\nC2H5OH:\t%d\r\nNO2:\t%d\r\n",
//			(uint32_t)(gas_meas.carbonMonoxide*100), (uint32_t)(gas_meas.methane*100), (uint32_t)(gas_meas.hydrogen*100),
//			(uint32_t)(gas_meas.ammonia*100), (uint32_t)(gas_meas.ethanol*100), (uint32_t)(gas_meas.nitrogenDioxide*100));

	MOX_Data_Update(&gas_meas);
}

/**
 * @brief Sending environmental and motion data
 * @param None
 * @retval None
 */
void SendEnvMotionData(){
	SendEnvironmentalData();
	Zeros_Update();
	SendMotionData();
}
